...
export function MeasurementPlanView({ plan }: { plan: MeasurementPlan }) {
  ...
  return <article className="measurement-proposal" aria-label="Measurement proposal">
    ...
    {!!plan.background?.length && <section className="proposal-section"><h4>Context</h4><ul className="plain-list">{plan.background.map((item) => <li key={item}>{item}</li>)}</ul></section>}
    ...
    <section className="proposal-section"><div className="proposal-section-heading"><h4>Measures</h4><span>{metrics.length}</span></div>
      <div className="measurement-table" role="table" aria-label="Proposed measures">{metrics.map(([name, metric]) => <div className="measurement-row" role="row" key={name}><div role="cell"><strong>{humanKey(name)}</strong>{metric.description && <p>{metric.description}</p>}{!!metric.evidence?.length && <small>Evidence: {metric.evidence.join(" · ")}</small>}</div><div role="cell"><span>{metric.direction === "max" ? "Higher is better" : metric.direction === "min" ? "Lower is better" : "Direction not set"}</span><small>{[metric.aggregation, metric.unit].filter(Boolean).join(" · ") || "No unit"}</small></div><div role="cell">{metric.weight !== undefined && <span>Weight {formatValue(metric.weight)}</span>}<small>Missing: {metric.missing || "not specified"}</small>{!!metric.prerequisites?.length && <small>Requires: {metric.prerequisites.join(" · ")}</small>}</div></div>)}</div>
    </section>
    <div className="proposal-grid">
      <section className="proposal-section"><h4>Decision rule</h4><dl className="compact-definition"><div><dt>Mode</dt><dd>{scoring.mode ? humanKey(scoring.mode) : "Not specified"}</dd></div><div><dt>Primary measure</dt><dd>{scoring.primary || scoring.custom_metric || "Not specified"}</dd></div>{scoring.target !== undefined && <div><dt>Target</dt><dd>{formatValue(scoring.target)}</dd></div>}</dl></section>
      <section className="proposal-section"><h4>Evaluation source</h4><dl className="compact-definition"><div><dt>Plan</dt><dd>{evaluation.evaluation_id ? "Reuse reviewed evaluation" : evaluation.mode === "reuse" ? "Reuse existing evaluator source" : "Create an evaluator"}</dd></div>{evaluation.evaluation_id && <div><dt>Evaluation</dt><dd><code>{evaluation.evaluation_id}</code></dd></div>}{evaluation.framework && <div><dt>Framework</dt><dd>{evaluation.framework}</dd></div>}{evaluation.entrypoint && <div><dt>Entry point</dt><dd><code>{evaluation.entrypoint}</code></dd></div>}{evaluation.command && <div><dt>Command</dt><dd><code>{typeof evaluation.command === "string" ? evaluation.command : evaluation.command.argv.join(" ")}</code></dd></div>}{evaluation.scorer && <div><dt>Scorer</dt><dd>{evaluation.scorer}</dd></div>}{evaluation.dataset_snapshot_id && <div><dt>Dataset</dt><dd><code>{evaluation.dataset_snapshot_id}</code></dd></div>}{Boolean(Object.keys(evaluation.output_mapping || {}).length) && <div><dt>Output mapping</dt><dd><code>{JSON.stringify(evaluation.output_mapping)}</code></dd></div>}</dl></section>
    </div>
...
function EvaluationResultView({ result }: { result: EvaluationResult }) {
  ...
  return <>
    ...
    <section className="result-section"><div className="result-section-heading"><div><p className="eyebrow">Evaluation definition</p><h4>Measures and review</h4></div><span>{metrics.length} measure{metrics.length === 1 ? "" : "s"}</span></div><dl className="compact-definition"><div><dt>Evaluation</dt><dd><code>{result.evaluation_id}</code></dd></div><div><dt>Independent review</dt><dd>{result.review_verdict ? humanKey(result.review_verdict) : "Not complete"}</dd></div><div><dt>Review branch</dt><dd>{result.review_branch || "Not prepared"}</dd></div>{Object.entries(result.coverage || {}).map(([name, value]) => <div key={name}><dt>{humanKey(name)}</dt><dd>{displayValue(value)}</dd></div>)}</dl>{metrics.length ? <div className="measurement-table" role="table" aria-label="Evaluation measures">{metrics.map(([name, metric]) => <div className="measurement-row" role="row" key={name}><div role="cell"><strong>{humanKey(name)}</strong>{metric.description && <p>{metric.description}</p>}</div><div role="cell"><span>{metric.direction === "max" ? "Higher is better" : metric.direction === "min" ? "Lower is better" : "Direction not set"}</span><small>{metric.aggregation || "Aggregation not set"}</small></div><div role="cell"><span>{metric.unit || "No unit"}</span><small>Missing: {metric.missing || "not specified"}</small></div></div>)}</div> : <p className="quiet-surface">No accepted metric definition is retained yet.</p>}</section>
