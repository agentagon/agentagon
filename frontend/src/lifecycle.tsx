...
import { Button, PageHeader, Status } from './components';
import { useAgents, useAssistants, useConnectors, useTask } from './hooks';
import { loadOperationDraft, operationBinding, useSessionOperation } from './session';
import type { Agent, Project } from './types';

type Setup = { state: string; task_state?: string; scope: Record<string, unknown>; task_id?: string; limitations?: string[] };
...
function assessmentState(state: string) {
  if (state === 'queued') return { title: 'Project analysis is queued', detail: 'Waiting for coding backend capacity. The task has not started yet.' };
  if (['running', 'analyzing'].includes(state)) return { title: 'Project analysis is running', detail: 'Discovered agents appear as evidence is retained.' };
  if (state === 'needs_input') return { title: 'Project analysis needs your input', detail: 'Open the task to answer its pending question. Existing discoveries are available for review.' };
  if (['paused', 'interrupted'].includes(state)) return { title: 'Project analysis is interrupted', detail: 'Work is stopped. Open the task to review available recovery actions and retained discoveries.' };
  if (['failed', 'stopped'].includes(state)) return { title: 'Project analysis stopped', detail: 'Review the stopping reason and any discoveries already retained before starting another attempt.' };
  if (state === 'cancelled') return { title: 'Project analysis was cancelled', detail: 'Any discoveries already retained remain available for review.' };
  if (state === 'completed_with_limits' || state === 'partial') return { title: 'Project assessed with limits', detail: 'Review the retained discoveries and coverage limits before deciding what to maintain.' };
  if (['complete', 'completed'].includes(state)) return { title: 'Project analysis is complete', detail: 'Review the discovered agents, then choose one useful action.' };
  return { title: state === 'ready' ? 'Project is ready to analyze' : 'Analyze this project', detail: 'Discover the agents in this folder. Production traces are optional.' };
}
...
export function OnboardingPage({ project }: { project: Project }) {
  ...
  const [detectionGeneration, setDetectionGeneration] = useState(0);
  const operation = useSessionOperation(`agentagon.detect:${project.id}`, operationBinding({ projectId: project.id, snapshot_ids: [], detectionGeneration }), {});
  const detect = useMutation({
    mutationFn: () => post<{ count: number; limitations: string[] }>(projectPath(project.id, '/agents/detect'), { operation_id: operation.operation }),
    onSuccess: async result => {
      await query.invalidateQueries({ queryKey: ['projects', project.id] });
      operation.clear(); setDetectionGeneration(value => value + 1);
      if (result.count) navigate(`/projects/${project.id}/agents`, { state: { detectionResult: result } });
    },
  });
  return <>
    <PageHeader eyebrow={project.name} title="Find your agents" />
    <section className="settings-card form detect-agents-card">
      <p>Detect agents in this folder and start improving them.</p>
      <code className="project-folder">{project.path}</code>
      <div className="form-actions"><Button disabled={detect.isPending} onClick={() => detect.mutate()}>{detect.isPending ? 'Detecting agents…' : 'Detect agents'}</Button></div>
      {detect.error && <p className="error-banner" role="alert">{detect.error.message}</p>}
      {detect.data && !detect.data.count && <div role="status"><p>No agents were detected in this folder. Add an agent manually or analyze code and traces below.</p><Link to={`/projects/${project.id}/agents`}>Add an agent</Link>{!!detect.data.limitations.length && <ul className="plain-list">{detect.data.limitations.map(item => <li key={item}>{item}</li>)}</ul>}</div>}
    </section>
    <details className="advanced-actions" open={advanced} onToggle={event => setAdvanced(event.currentTarget.open)}><summary>Analyze code and traces</summary>{advanced && <AdvancedAssessment project={project} />}</details>
  </>;
}

function AdvancedAssessment({ project }: { project: Project }) {
  const query = useQueryClient();
  const navigate = useNavigate();
  const setup = useQuery({ queryKey: ['projects', project.id, 'onboarding'], queryFn: () => api<Setup>(projectPath(project.id, '/onboarding')) });
  const assessmentTask = useTask(project.id, setup.data?.task_id);
  ...
  const analyze = useMutation({ mutationFn: async () => { await save.mutateAsync(); return post<{ task_id: string }>(projectPath(project.id, '/tasks'), { operation_id: operationDraft.operation, ...assessmentRequest }); }, onSuccess: task => { const taskId = task.task_id; if (!taskId) throw new Error("The task did not return its identity. Retry with the same request."); operationDraft.clear(); navigate(`/projects/${project.id}/tasks/${taskId}?view=running`); } });
  ...
  const assessmentStatus = assessmentTask.data?.state || setup.data?.task_state || setup.data?.state || '';
  const assessmentPresentation = assessmentState(assessmentStatus);
  const assessmentActive = ['queued', 'running', 'analyzing', 'needs_input'].includes(assessmentStatus);
  ...
  return <>
    <PageHeader title={`Start with ${project.name}`} actions={<><Link className="button button-secondary" to={`/projects/${project.id}/home`}>Open workspace</Link><Button type="submit" form="project-analysis-form" disabled={analyze.isPending || traceSelectionUnavailable || !setup.data || assistants.isLoading || assessmentActive}>{analyze.isPending ? 'Starting analysis…' : setup.data?.task_id ? 'Analyze again' : 'Analyze project'}</Button></>}><p>Review the source, optional trace scope, and limits before analysis.</p></PageHeader>
    {(save.error || analyze.error) && <p className="error-banner" role="alert">Analysis could not start: {(save.error || analyze.error)?.message}</p>}
    {setup.error && <div className="error-banner" role="alert"><p>Saved setup could not be loaded: {setup.error.message}</p><Button tone="secondary" onClick={() => setup.refetch()}>Retry setup</Button></div>}
    {setup.data?.task_id && <section className={`onboarding-outcome outcome-${assessmentStatus}`}><div><p className="eyebrow">{assessmentActive ? 'Current activity' : 'Latest outcome'}</p><h2>{assessmentTask.isLoading ? 'Reading assessment status…' : assessmentPresentation.title}</h2><p>{assessmentTask.error ? 'The latest task status could not be loaded. Open the task to inspect its retained evidence.' : assessmentPresentation.detail}</p></div><div className="onboarding-outcome-actions"><Link className="button button-secondary" to={`/projects/${project.id}/tasks/${setup.data.task_id}?view=all`}>View assessment</Link>{!assessmentTask.isLoading && <Link className="button button-primary" to={`/projects/${project.id}/agents`}>Open agents</Link>}</div></section>}
