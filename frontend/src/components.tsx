  ...
  return <section className="task-detail" aria-label="Task details">
    ...
    {task.data && <><div className="task-meta"><Status value={task.data.state} /><span>{task.data.agent_name || "Agent"}</span>{task.data.goal_name && <span>{task.data.goal_name}</span>}</div>
      <div className="conversation" aria-live="polite">
        ...
        {!task.data.conversation.length && <p className="quiet-copy">{task.data.state === "queued" ? "Waiting for coding assistant capacity. This task continues in the background." : task.data.state === "running" ? "Running in the background." : "Waiting for an update."}</p>}
      </div>
