...
The dashboard gives you a local browser view of an application’s baseline history, evaluations, audits and fix experiments. Use it to inspect evidence and compare results while your coding agent works.
...
|---|---|
| Baselines | Branch and exact commit, evaluator version, benchmark score and separate recent-trace score. |
| Settings | Existing execution/provider configuration and credential references. |
...
## Enable controls deliberately

Ask your coding agent to open **ag:dashboard** with controls enabled when you want to rerun a baseline, update settings or control a fix run.
...
## Rerun a saved baseline

Choose an explicit rerun in the Baselines view. It creates a bounded CLI-backed job using the saved evaluator, behavior definitions, scoring, judge and execution settings against current committed code. It creates a new measurement identity and keeps the earlier history. Saved authorized provider settings refresh traces with a window ending at the rerun time; absent or partial acquisition stays visible.

The job persists when the browser disconnects. Work requiring a coding host, provider tool, grading or review stays pending with a next action. Opening the dashboard or reading status never starts execution or reconnects to a remote runner. See [baseline semantics](baselines.md).

## Manage settings and export results

The Settings view reuses validated configuration operations for provider connections and existing preferences. Credential fields store environment-variable references. Project overrides and explicit invocation settings keep their existing precedence.

Baseline and run reports are available as readable Markdown and JSON. They include goals, scoring, behavior outcomes, evidence summaries and source/evaluator identities while excluding raw traces, private inputs and credentials. Fix comparisons show the winner and up to two qualifying alternatives; an unavailable measurement remains unavailable.
