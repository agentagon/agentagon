...
**ag:audit**, **ag:fix**, and checkout-scoped **ag:setup** automatically open the dashboard when they start or resume. The coding host keeps its server running in the background, reuses the same session's server and tab for that application, and selects the active audit, evaluation or fix run once its ID is available. Nested skills reuse that dashboard too. User-wide setup without an application directory has no dashboard to open.
