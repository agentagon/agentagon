...
An authenticated coding agent is required before discovery. Discovery first performs the bounded local scan, then asks the default coding agent to review only those candidates. Its structured response removes false positives and duplicates, improves names and supplies a concise responsibility for every retained agent; deterministic validation prevents it from adding paths or confirming agents. Coding-review failure stops discovery. An optional saved choice may also read at most 100 recent root-trace metadata records from one tested connection assigned to the project. That lookup covers the last seven days and excludes trace inputs, outputs and child spans. Trace matches are advisory identity evidence, not evidence of behavioral correctness.
