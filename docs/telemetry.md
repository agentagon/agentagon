...
|---|---|---|
| `workflow_started` | A built-in workflow started | Workflow name and host enum |
...
These are event counts, not unique-user metrics or evidence that guidance caused an improvement. Event reporting depends on internal instrumentation. Public ingestion can receive fabricated events, and outages or queue limits can lose events.
