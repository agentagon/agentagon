...
On retention, artifact bytes are stored once by content hash in `.agentagon/evidence/`; trial results refer to these binary files. The retained result and local runner caches share the same bytes through hard links where supported, with atomic replacement for cache updates. Filesystems without hard-link support keep separate result copies. Once the final result is durable and the worker has stopped, redundant progress, log and benchmark-output copies are removed. Ownership and request records remain for safe recovery. Existing evidence is not rewritten, and downloads continue to support older results with embedded Base64 artifacts.
