...
Use Python 3.12+, Git and Node.js 22 on macOS or Linux. Node runs JavaScript syntax checks and event-helper tests; it is not required for the README example. You need network access to install dependencies.
...
If you already created `.venv` for the quickstart, reuse it and start at the activation command. Editable installation makes Python source changes available without reinstalling. The `dev` extra supplies pytest, Ruff and the build frontend; versions and dependency bounds are defined in [pyproject.toml](pyproject.toml).
