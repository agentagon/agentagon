...
Use this procedure for named evaluation and fix execution profiles. Inspect `settings.profiles` and reuse an appropriate profile when one already exists. Save a complete profile with `agentagon --workspace CHECKOUT setup --scope SCOPE --profile NAME --profile-file PROFILE_JSON`. Do not combine a profile change with `--set` or `--unset`.

A user profile applies across checkouts. A project profile replaces the complete same-named user profile rather than merging fields. Preserve every existing field when editing one. Follow the [fix profile contract](../../fix/references/contract.md) for the full JSON shape.
...
Existing runs retain their frozen profile and scan settings. Change an active search policy through `fix steer` using [ag:fix](../../fix/SKILL.md); do not edit canonical state or imply Setup rewrites historical results.
