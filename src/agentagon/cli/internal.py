...
@click.group(name="_internal", hidden=True)
...
def fix_start(
    ...
    from agentagon.capabilities.experiments import engine, preparation
    ...
    if not spec_path and not evaluation_id:
        return {
            ...
            "goal": goal,
            "issue_ids": list(issue_ids),
            "next_action": "Continue the managed workflow: inspect existing benchmarks and authorized preparation limits, then use eval start/check/freeze; if measurement is unavailable, report the blocker; unmeasured patches require an explicit user request.",
        }
    specification = (
...
def fix_report(path: Path, run_id: str) -> dict:
    ...
    from agentagon.capabilities.experiments.store import export_report
...
def fix_steer(path: Path, run_id: str, control_file: Path) -> dict:
    ...
    from agentagon.capabilities.experiments.controls import submit as submit_control
...
def fix_cleanup(
    ...
    from agentagon.capabilities.experiments import cleanup
...
def fix_ship(
    ...
    from agentagon.capabilities.experiments.delivery import ship
...
@main.command("repair-bind")
@click.option("--run", "run_id", required=True)
@click.option(
    "--context", type=click.Path(exists=True, dir_okay=False, path_type=Path), required=True
)
@click.pass_obj
@output
def repair_bind(path, run_id, context):
    """Freeze a managed focused repair before its first execution."""
    from agentagon.workflows.fix.handler import configure

    return configure(Workspace(path), run_id, load_json(context))
