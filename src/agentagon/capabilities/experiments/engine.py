...
from agentagon.capabilities.experiments import checkouts, evaluation, evidence, runners, search
from agentagon.capabilities.experiments.spec import validate_limits, validate_spec
from agentagon.capabilities.experiments.store import list_runs, load_run, locked, run_dir, save_run
...
def _update(data: dict) -> None:
    ...
    for round_ in data["rounds"]:
        ...
        if "branches" in round_:
            from agentagon.capabilities.experiments.orchestration import update_round
    ...
    if data["state"] != "stopped":
        ...
        if (
            ...
            from agentagon.capabilities.experiments.orchestration import stagnation_state
...
def start(
    ...
    *,
    ...
    from agentagon.memory.execution import freeze_spec

    spec = freeze_spec(workspace, copy.deepcopy(spec))
...
def new(
    ...
    with locked(workspace, run_id):
        ...
        if data.get("focused_repair"):
            ...
            BudgetLedger(workspace, data["budget_id"]).release_preparation()
...
def _continue(data: dict, continuation: bool, limits: dict | None) -> None:
    if limits is not None:
        if data.get("focused_repair") and limits != data["limits"]:
            raise AuditError("focused repair limits are frozen; start a new task for a new budget")
        if not continuation:
...
def _validate_result(
    ...
    from agentagon.capabilities.experiments.spec import finite
...
def _collect(workspace: Workspace, data: dict, candidate: dict, trial: dict, result: dict) -> None:
    ...
    try:
        ...
        if missing:
            from agentagon.capabilities.experiments import grading
...
def _summarize(data: dict, candidate: dict) -> None:
    ...
    if data["spec"].get("scoring"):
        from agentagon.capabilities.experiments.scoring import candidate_score
...
def _select_locked(workspace: Workspace, data: dict, candidate_id: str) -> None:
    ...
    if data["spec"].get("scoring"):
        from agentagon.capabilities.experiments.scoring import qualifying
...
def status(
    ...
    from agentagon.capabilities.experiments import controls, learning
