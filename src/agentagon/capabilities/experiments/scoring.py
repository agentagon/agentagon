...
from agentagon.capabilities.experiments.spec import finite, path
...
def validate(definition: dict, spec: dict | None = None) -> dict:
    ...
    if judge:
        ...
        if "trace_command" in judge:
            from agentagon.capabilities.experiments.spec import command
...
def qualifying(data: dict) -> list[dict]:
    ...
    from agentagon.capabilities.experiments.evaluation import invalidated
