...
def validate_profile(value: dict) -> dict:
    ...
    if "search" in value:
        from agentagon.capabilities.experiments.search import validate_policy

        value["search"] = validate_policy(value["search"])
    if "scans" in value:
        from agentagon.capabilities.experiments.learning import (
            validate_limits as validate_scan_limits,
        )

        value["scans"] = validate_scan_limits(value["scans"])
    if "evidence" in value:
        from agentagon.capabilities.experiments.evidence import (
            validate_limits as validate_evidence_limits,
        )

        value["evidence"] = validate_evidence_limits(value["evidence"])
    if "orchestration" in value:
        from agentagon.capabilities.experiments.orchestration import validate_settings
...
def validate_spec(value: dict) -> dict:
    ...
    if "scoring" in value:
        from agentagon.capabilities.experiments.scoring import validate
