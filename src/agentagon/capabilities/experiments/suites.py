"""Immutable, isolated final measurements across retained application goals."""
...
from agentagon.capabilities.experiments import baselines, engine, evaluation, preparation, scoring
...
def _manifest(value):
    ...
    ids = [m.get("goal_id") for m in members]
    if (
        None in ids
        or len(ids) != len(set(ids))
        or (manifest.get("goal_id") is not None and manifest["goal_id"] not in ids)
    ):
        raise AuditError("suite requires unique goals including its primary objective")
    return manifest
...
def bind(workspace, run_id, manifest, *, finalist_count=3):
    ...
    with _locked(workspace, run_id), run_locked(workspace, run_id):
        ...
        definitions = {m["goal_id"]: _definition(workspace, m) for m in canonical["members"]}
        active_spec = definitions[canonical["goal_id"]]["spec"] if canonical["goal_id"] else None
        ...
        if active_spec is not None and executable(data["spec"]) != executable(active_spec):
...
def _measurement(workspace, state, member, role, host, host_handler):
    ...
    definition = state["definitions"][member["goal_id"]]
    source = (
    ...
    key = member["goal_id"] + ":" + role
    entry = state["measurements"].setdefault(
        key, {"execution_run_id": child_id, "role": role, "goal_id": member["goal_id"]}
    )
...
def _observed(workspace, state, member, role):
    entry = state["measurements"].get(member["goal_id"] + ":" + role)
...
def _result(workspace, state):
    ...
    for member in state["manifest"]["members"]:
        ...
        primary = member["goal_id"] == state["manifest"]["goal_id"]
        ...
        rows.append(
            {
                ...
                "passed": passed,
                "executions": {
                    role: state["measurements"][member["goal_id"] + ":" + role]["execution_run_id"]
...
def advance(workspace, run_id, *, candidate_id=None, host_handler=None):
    with _locked(workspace, run_id):
        ...
        engine._verified_evidence(workspace, data, candidate)
        if not candidate.get("verification_of") and not data.get("focused_repair"):
