...
from agentagon.capabilities.experiments import checkouts, evaluation, runners
from agentagon.capabilities.experiments.evidence import DEFAULT_LIMITS, read_result, retain_result
...
def check(workspace: Workspace, evaluation_id: str, plan: dict) -> dict:
    ...
    with locked(workspace, evaluation_id) as root:
        ...
        if isinstance(plan, dict) and isinstance(plan.get("spec"), dict):
            plan["spec"] = freeze_spec(workspace, plan["spec"])
