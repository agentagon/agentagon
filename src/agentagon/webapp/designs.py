...
class Designs:
    ...
    def _identity(self, project_id, agent_id, focus_id):
        self.catalog.focus(project_id, agent_id, focus_id)
        return "design_" + hashlib.sha256(f"{agent_id}:{focus_id}".encode()).hexdigest()[:24]

    def get(self, project_id, agent_id, focus_id):
        key = self._identity(project_id, agent_id, focus_id)
        return self.state.db.get_record(project_id, "measurement_designs", key)

    def accepted(self, project_id, agent_id, focus_id):
        focus = self.catalog.focus(project_id, agent_id, focus_id)
        key = focus.get("accepted_design_id")
        if not key:
            return None
        record = self.state.db.get_record(project_id, "accepted_designs", key)
        if not record or record["agent_id"] != agent_id or record["focus_id"] != focus_id:
    ...
    def save(self, project_id, agent_id, focus_id, payload):
        key = self._identity(project_id, agent_id, focus_id)
    ...
    def accept(self, project_id, agent_id, focus_id, payload):
        if (
        ...
        draft = self.get(project_id, agent_id, focus_id)
        ...
        with self.state.db.transaction() as tx:
            focus = tx.get_record(project_id, "focuses", focus_id)
            if not focus or focus["agent_id"] != agent_id:
                raise AuditError("focus not found for this application agent")
            key = "design_" + hashlib.sha256(f"{agent_id}:{focus_id}".encode()).hexdigest()[:24]
            ...
            if draft["state"] == "accepted":
                return tx.get_record(project_id, "accepted_designs", focus["accepted_design_id"])
            accepted = {
                **draft,
                "id": version_id,
                "state": "accepted",
            ...
            focus.update(accepted_design_id=version_id, version=focus["version"] + 1)
            saved = tx.put_record(
                project_id, "focuses", focus_id, focus, expected_revision=focus["revision"]
            )
            tx.put_record(
                project_id,
                "focus_versions",
                version_id,
                {"id": version_id, "focus_id": focus_id, "definition": saved, "created_at": now()},
            )
            tx.put_record(
                project_id,
                "measurement_designs",
