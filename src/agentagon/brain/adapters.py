...
class _Run:
    ...
    def message(self, text, *, phase=None):
        ...
        # cannot leak into separately persisted events. Tool activity remains available
        # as diagnostic data without becoming dashboard progress.
        ...
        self.emit(
            {
                ...
                **({"phase": phase} if isinstance(phase, str) and phase else {}),
            }
        )
...
def run_agent(
    ...
    for key, value in environment.items():
        ...
        if key == "AGENTAGON_MEMORY_SNAPSHOT" and not path.is_file():
            raise AuditError("Coding-agent memory snapshot must be an existing file.")
