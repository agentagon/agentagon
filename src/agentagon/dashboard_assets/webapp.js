...
async function discoverAgents() {
  ...
  body.append(el("p", "", "Inspect this project's code and already imported trace metadata for candidate agents. This does not run your application or fetch new provider records."));
...
function focusForm(route = "goal") {
  ...
  const noIssues = el("div", "warning"); noIssues.append(el("p", "", "This agent has no saved issues. Start from a goal and define how to measure it."), quiet("Start from a goal", () => { source.input.value = "goal"; update(); }));
...
function launch(kind, preset = {}, acceptedPlan = false) {
  ...
  body.append(el("p", "", {design: "Your coding agent inspects requirements, code and available evidence, then saves a draft for your review. It does not prepare or run an evaluator until you accept a plan.", eval: "Describe expected behavior. Your agent prepares or assesses the evaluator, validates it and requests independent review.", fix: "Agree on an outcome, keep the evaluator fixed and compare verified candidates within your limits.", baseline: "Measure the current committed application with a frozen evaluator. This creates a new measurement."}[kind]));
  ...
  const codePaths = field("Permitted edit paths", {value: (preset.permitted_paths || []).join(", "), placeholder: "src/agent, tests", hint: "Comma-separated paths. Your agent confirms the edit boundary before changing code."});
...
function renderAgent() {
  ...
  if (!selected) { const wrap = el("div", "agent-empty"); wrap.append(el("span", "", "✳"), el("h3", "", "Work with your agent here."), el("p", "", "Set a goal, prepare an eval or improve a behavior. Messages, questions and approvals stay with the task.")); content.append(wrap); return; }
