...
Prefix these subcommands with `agentagon _internal --workspace CHECKOUT`. Uppercase arguments are placeholders; quote substituted values as data, never shell expressions.
...
Read the submission schema returned by the task context’s `resources` paths for finding, flag-disposition, and group fields absent from the template. Evidence IDs come from the packet index. A reviewed healthy trace has `findings: []` and still accounts for every flag.
