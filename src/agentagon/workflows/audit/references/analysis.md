...
Read the catalog returned by the task context’s `resources` paths; use its shared judgment rules, exact facets, values, evidence rules, and counterexamples. Review every packet subject/facet exactly once, using `not_applicable` when appropriate rather than forcing a positive or negative value.
