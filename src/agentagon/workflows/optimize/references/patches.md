...
2. Write a plan in ignored `.agentagon/` storage with `editable_paths`, `checks` (each has `id`, `argv` and optional `cwd`), `timeout_seconds`, and `max_attempts`. These are explicit authorized limits. An empty check list requires a nonempty `no_checks_reason` and must be explained in the result; do not invent a dummy benchmark. Use the task context’s `resources` paths for the patch plan contract.
