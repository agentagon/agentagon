...
All commands use `agentagon _internal --workspace ORIGIN_CHECKOUT`. Canonical run, snapshot and trial records, plus exported report snapshots, stay in that checkout's ignored `.agentagon/`, even when a candidate worktree or remote runner executes checks.

Use the task context’s `resources` paths to locate installed JSON contracts. Its `contracts` directory contains `fix-profile.json`, `fix-spec.json`, `fix-review.json` and `fix-control.json`; do not assume repository-relative schema paths exist in a installed package.
...
```sh
agentagon _internal --workspace CHECKOUT fix start --spec SPEC_JSON --profile PROFILE --goal GOAL
agentagon _internal --workspace CHECKOUT fix run RUN_ID
agentagon _internal --workspace CHECKOUT fix run RUN_ID --review-file BASELINE_REVIEW_JSON
agentagon _internal --workspace CHECKOUT fix new RUN_ID --hypothesis HYPOTHESIS --author AUTHOR
agentagon _internal --workspace CHECKOUT fix run RUN_ID CANDIDATE_ID
...
agentagon _internal --workspace CHECKOUT fix report RUN_ID
agentagon _internal --workspace CHECKOUT fix select RUN_ID CANDIDATE_ID
agentagon _internal --workspace CHECKOUT fix stop RUN_ID
agentagon _internal --workspace CHECKOUT fix steer RUN_ID --control-file CONTROL_JSON
agentagon _internal --workspace CHECKOUT fix ship RUN_ID
agentagon _internal --workspace CHECKOUT fix report RUN_ID
```
