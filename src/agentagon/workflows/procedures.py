...
def write_context(workspace, job):
    ...
    if job["options"].get("trace_snapshot_id"):
        from agentagon.workflows.discover.handler import evidence

        context["trace_evidence_path"] = str(
            workspace.root / evidence(workspace, job["options"]["trace_snapshot_id"])
        )
    if job["options"].get("issue_id"):
        from agentagon.domain.issues import get_issue

        context["issue"] = get_issue(workspace, job["options"]["issue_id"])
    path = private_directory(workspace, "task-inputs") / f"{job['id']}-context.json"
...
def prompt(workspace, job):
    config = workspace.state / "runtime" / "task-inputs" / f"{job['id']}-config.json"
    command = (
        f"env AGENTAGON_CONFIG={shlex.quote(str(config))} {shlex.quote(sys.executable)} "
        f"-m agentagon _internal --workspace {shlex.quote(str(workspace.root))}"
    )
    refs = {p: str(resource_path(p)) for p in REFERENCES[job["kind"]]}
    instructions = instruction_text(job["kind"])
    ...
    if job["kind"] == "fix":
        instructions += f" Immediately after fix start, before any trial, run `repair-bind --run RUN_ID --context {shlex.quote(str(manifest))}`. This freezes the shared budget and any regression suite. After candidate review, run `suite run --run RUN_ID --candidate CANDIDATE_ID` when the context includes suite_manifest. Complete its managed review handoffs before selection. "
    ...
    if job["kind"] == "optimize" and job["options"].get("suite_manifest"):
    ...
    return f"""You are the managed brain for a user-requested Agentagon workflow task.
The shared service owns task lifecycle, pending questions and approvals. Do not open dashboards, install
plugins, send skill telemetry, bind native continuation hooks, or launch another supervisor.
The reference files below are the authoritative bundled runtime procedures.
...
Consult the context's frozen improvement_memory before deciding an approach. Return `lessons` as a
list of supplied memory references with id, version, decision (used or rejected), and reason.
These are advisory lessons; verification and scope remain governed by evidence and code.
Also return a learning object with hypothesis, action, result, failed_approaches (a list), and
uncertainty. Describe what was actually attempted and distinguish reported interpretation from
verified evidence. Failed approaches and inconclusive results are useful outcomes.
...
def reflection_handoff(workspace, job, text):
    ...
    if handoff is None:
        return None
    if job["kind"] != "optimize" or not isinstance(handoff, dict):
...
def review_handoff(workspace, job, text):
    ...
    if job["kind"] == "fix" and "candidate_id" in handoff:
        from agentagon.workflows.fix.handler import review_handoff as candidate_review

        return candidate_review(
            workspace, job, handoff, handoff.get("run_id") or result.get("run_id")
        )
    if "evaluation_id" in handoff:
        if job["kind"] == "fix" and job["workflow_ids"].get("run_id"):
            raise AuditError("repair evaluator is frozen; start a new task to change it")
        if job["kind"] not in {"eval", "optimize", "fix"}:
        ...
        if job["kind"] not in {"optimize", "fix", "baseline"}:
...
{resource_path("workflows/optimize/references/evaluation.md")}. For an evaluator, also read
{resource_path("workflows/evaluate/references/preparation.md")}.
...
{resource_path("workflows/shared/review-evaluation.md")}.
...
def apply_review(workspace, job, task):
    ...
    if task["kind"] == "candidate":
        ...
        engine.run(workspace, task["run_id"], task["candidate_id"], review=review)
    elif task["kind"] == "evaluation":
...
def freeze_settings(workspace, job):
    ...
    config = private_directory(workspace, "task-inputs") / f"{job['id']}-config.json"
    settings = {
...
def validate_run(workspace, job, candidate):
    from agentagon.capabilities.experiments import preparation
    from agentagon.capabilities.experiments.store import load_run
    ...
    verify_snapshot(workspace, job, record)
    _validate_limits(job, record)
    _validate_paths(workspace, job, record)
...
def validate_result(workspace, job, output):
    ...
    from agentagon.memory.lessons import learning
    from agentagon.memory.lessons import validate as validate_lessons

    lessons = validate_lessons(job, raw_result)
    learned = learning(raw_result)
    if job["kind"] in {"discover", "design", "assess", "observe"}:
        state, result, action = handler(job["kind"]).accept(workspace, job, raw_result)
    ...
    record, state, complete = handler(job["kind"]).validate(workspace, job, candidate, result)
